...
# read the tables
# Optimized: Use data.table::fread for 10-20x faster loading
# NOTE: fread() defaults to check.names=FALSE, so we must explicitly set TRUE
# to convert "Sample ID" -> "Sample.ID" to match existing code expectations
Sample_table <- as.data.frame(fread(Sample_table_file, header = TRUE, sep = '\t', stringsAsFactors = FALSE, quote = "", dec=",", check.names=TRUE))
SNP_table <- as.data.frame(fread(SNP_table_file, header = TRUE, sep = '\t', stringsAsFactors = FALSE, quote = "", dec=",", check.names=TRUE))
Full_table <- as.data.frame(fread(Full_table_file, header = TRUE, sep = '\t', stringsAsFactors = FALSE, quote = "", dec=",", check.names=TRUE))
...
if(nmale > 0){
  if(ncores > 1){
    ...
    id_SNP_to_exclude <- which(bool_SNP_to_exclude)
  }
...
# Optimized: Convert to matrix once for faster access
GT_matrix <- as.matrix(GT_samples)
...
if(ncores > 1){
  ...
  # Export necessary objects and functions to cluster workers
  clusterExport(cl, c("GT_matrix", "nsamples", "compareGT"), envir = environment())
  GT_comp <- foreach(i = 1:(nsamples-1), .combine = 'c')%dopar%{
    print(i)
    # Vectorized comparison for all pairs at once
    sapply((i+1):nsamples, function(j) {
      compareGT(GT_s1 = GT_matrix[,i], GT_s2 = GT_matrix[,j])
    })
  }
  stopCluster(cl)
} else {
  # Sequential processing for single core (memory efficient)
  GT_comp <- unlist(lapply(1:(nsamples-1), function(i) {
    print(i)
    sapply((i+1):nsamples, function(j) {
      compareGT(GT_s1 = GT_matrix[,i], GT_s2 = GT_matrix[,j])
    })
...
# Optimized: Use data.table for much faster sorting (vectorized operations)
LRR_dt <- as.data.table(LRR_table_temp)
BAF_dt <- as.data.table(BAF_table_temp)
GT_dt <- as.data.table(GT_table_temp)

# Sort by chromosome and position in one operation (much faster than loop)
setorder(LRR_dt, CHROM, POS)
setorder(BAF_dt, CHROM, POS)
setorder(GT_dt, CHROM, POS)

# Convert back to data.frame for compatibility with downstream code
LRR_table <- as.data.frame(LRR_dt)
BAF_table <- as.data.frame(BAF_dt)
GT_table <- as.data.frame(GT_dt)
